...
def test_build_model_creates_instance(user1):

    instance = DummyHelperService._build_model(
        owner=user1,
        name="Example",
    )
...
def test_apply_scalar_updates_changes_only_configured_fields(tagged1_item1):
...
def test_save_calls_full_clean_and_persists(tagged1_item1):
    ...
    refreshed = DummyItem.objects.get(pk=tagged1_item1.pk)
...
def test_required_m2m_fields_are_required():
    ...
    with pytest.raises(BusinessRuleViolationError):
        ...
        DummyHelperService._validate_required_m2m_fields(
            validated_data={}
        )
