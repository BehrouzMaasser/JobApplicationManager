...
class TestCompanyEmailListView:
    ...
    def test_redirects_anonymous_user(self, client):
        response = client.get(reverse("company-email-list-web"))
    ...
    def test_authenticated_user_can_access(self, client, co1_ws1_user1):
        ...
        response = client.get(
            company_email_list_url(company_id=co1_ws1_user1.pk)
        )

        assert response.status_code == 200
        assert "emails" in response.context

    def test_authenticated_user_get_list(self, client, co_email1_co1_ws1_user1):
        client.force_login(co_email1_co1_ws1_user1.company.workspace.owner)

        response = client.get(
            company_email_list_url(company_id=co_email1_co1_ws1_user1.company.pk)
        )

        assert response.status_code == 200
        assert co_email1_co1_ws1_user1 in response.context["emails"]

    def test_list_only_returns_users_company_emails(
            self,
            client,
            user2,
        ...
        assert response.status_code == 200
        assert co_email1_co1_ws1_user1 not in response.context["emails"]
...
class TestCompanyEmailCreateView:
    ...
    def test_create_company_email_invalid_company_id(
            self,
            client,
            user1,
            workspace1_user1
...
class TestCompanyEmailDetailView:
    ...
    def test_authenticated_user_can_access(
        ...
        assert response.context["email"] == co_email1_co1_ws1_user1

    def test_user_cannot_view_other_users_company_email(
            self,
            client,
            user2,
            co_email1_co1_ws1_user1,
        ...
        response = client.get(
            reverse(
                "company-email-detail-web",
                kwargs={"pk": co_email1_co1_ws1_user1.pk},
            )
        )
...
class TestCompanyEmailUpdateView:
    ...
    def test_valid_post_updates_company_email(
        ...
        assert co_email1_co1_ws1_user1.title == "T1 Updated"
        assert co_email1_co1_ws1_user1.email == "email_updated@gmail.com"

    def test_user_cannot_update_other_users_company_email(
            self,
            client,
            user2,
            co_email1_co1_ws1_user1,
        ...
        assert co_email1_co1_ws1_user1.title != "Cant Update"
        assert co_email1_co1_ws1_user1.email != "cant_update@gmail.com"
...
class TestCompanyEmailDeleteView:
    ...
    def test_user_cannot_delete_other_users_company_email(
            self,
            client,
            user2,
            co_email1_co1_ws1_user1,
        ...
        assert CompanyEmail.objects.filter(pk=co_email1_co1_ws1_user1.pk).exists()
